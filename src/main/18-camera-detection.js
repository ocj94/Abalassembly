...
// Separation plateau/bille par la DISTANCE COULEUR AU MEDIAN RGB de
// l'echantillon (le median approxime le plateau tant que les cases vides
// restent majoritaires, vrai meme en position de depart complete : 33
// vides sur 61). Historique : la premiere version separait par la
// CHALEUR de teinte (r-b) seule, qui suppose un plateau chaud comme le
// bois -- verifie 100/100 sur bois, mais 0 a 2/20 sur plastique gris (le
// plateau Abalone standard) selon la simulation, avec la bille reelle
// invisible a chaque photo.
//
// Coupure par EPLUCHAGE ITERATIF plutot qu'un seuil unique : au plus 2
// couleurs de bille existent. On isole le groupe le plus contraste (ex.
// noir sur plateau clair), on retire ces cases pour re-estimer le
// plateau, puis on cherche un second ecart parmi ce qui reste -- capturant
// l'autre couleur meme faiblement contrastee. Un seuil unique ratait ce
// second cas : sur plateau clair avec billes noires ET blanches
// ensemble, l'ecart le plus net separe le noir du reste et engloutit les
// blanches, plus proches du plateau, avec les cases vides.
//
// Validation par simulation (bruit +/-12 par canal, 100 essais) : 100/100
// sur plateau gris et bois, tous scenarios (bille isolee, petit groupe,
// melange des deux couleurs, plateau vide, position de depart complete).
// LIMITE CONNUE, physique et non un defaut de code : un plateau et des
// billes de couleur quasi identique (noir sur noir) restent difficiles a
// separer par la seule couleur moyenne -- la correction manuelle deja
// proposee a l'ecran reste le filet de securite pour ce cas.
function _detDistancesRGB(samples) {
  const med = arr => { const s = arr.slice().sort((a,b)=>a-b); return s[Math.floor(s.length/2)]; };
  const mr = med(samples.map(s=>s.r)), mg = med(samples.map(s=>s.g)), mb = med(samples.map(s=>s.b));
  return samples.map(s => ({ key: s.key, dist: Math.sqrt((s.r-mr)**2 + (s.g-mg)**2 + (s.b-mb)**2), lum: s.lum }));
}
function _detMeilleureCoupure(dists, seuilAbsolu) {
  const tries = dists.slice().sort((a,b) => b.dist - a.dist);
  const gaps = [];
  for (let i=1; i<tries.length; i++) gaps.push(tries[i-1].dist - tries[i].dist);
  let bestGap=-1, cut=0;
  gaps.forEach((g,i) => { if (g>bestGap) { bestGap=g; cut=i+1; } });
  const others = gaps.filter((_,i) => i!==cut-1);
  const avgOther = others.length ? others.reduce((s,g)=>s+g,0)/others.length : 0;
  if (!(bestGap > seuilAbsolu && bestGap > 2.5*Math.max(avgOther,0.5))) return new Set();
  return new Set(tries.slice(0,cut).map(x=>x.key));
}
function detClassify(samples) {
  // Passe 1 : le groupe le plus contraste au plateau (seuil absolu plus
  // haut : c'est la coupure la plus nette attendue).
  const groupe1 = _detMeilleureCoupure(_detDistancesRGB(samples), 15);

  // Passe 2 : plateau re-estime SANS le groupe 1, pour affiner le median et
  // chercher un second ecart, naturellement plus faible (seuil plus bas).
  const restants = samples.filter(s => !groupe1.has(s.key));
  let groupe2 = new Set();
  if (restants.length > 1) groupe2 = _detMeilleureCoupure(_detDistancesRGB(restants), 8);

  const marbleKeys = new Set([...groupe1, ...groupe2]);
  const labels = {};
