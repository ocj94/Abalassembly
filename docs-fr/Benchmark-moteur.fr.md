🇬🇧 [English version](../docs-en/Benchmark-moteur.en.md)
...
**Lecture honnête** : l'efficacité s'effondre au-delà de la profondeur 3. Les coups racine sont répartis une fois pour toutes entre les workers, sans redistribution — un worker qui finit tôt attend pendant qu'un autre traîne. Voir [Moteur multi-worker](Moteur-multi-worker.fr.md) pour le détail, et une limite plus profonde encore qui n'est pas résolue.
...
Détail complet et interprétation : [Moteur expérimental](Moteur-experimental.fr.md).
...
- **Une comparabilité parfaite entre workers** : limite réelle, documentée dans [Moteur multi-worker](Moteur-multi-worker.fr.md).
